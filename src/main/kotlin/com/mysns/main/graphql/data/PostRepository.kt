...
interface PostRepository : JpaRepository<Post, Long> {
    ...
    @Query(
        ...
        WHERE (p.authorId IN (SELECT u.id FROM User u WHERE u.privateAccount = false)
               OR p.authorId = :viewerId
               OR p.authorId IN (SELECT f.followeeId FROM Follow f WHERE f.followerId = :viewerId))
        ORDER BY p.createdAt DESC
        """,
    )
    fun findVisibleFeed(@Param("viewerId") viewerId: Long, pageable: Pageable): List<Post>
    ...
    @Query(
        ...
        WHERE (LOWER(p.content) LIKE LOWER(CONCAT('%', :q, '%'))
          ...
          AND (p.authorId IN (SELECT u.id FROM User u WHERE u.privateAccount = false)
               OR p.authorId = :viewerId
               OR p.authorId IN (SELECT f.followeeId FROM Follow f WHERE f.followerId = :viewerId))
        ORDER BY p.createdAt DESC
        """,
    )
    fun searchVisible(@Param("q") q: String, @Param("viewerId") viewerId: Long, pageable: Pageable): List<Post>
