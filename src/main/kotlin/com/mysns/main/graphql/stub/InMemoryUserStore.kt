...
class InMemoryUserStore(passwordEncoder: PasswordEncoder) {
    ...
    private val byUsername: MutableMap<String, Long> = ConcurrentHashMap()
    ...
    init {
        ...
        listOf(
            User(1, "alice", "Alice", "GraphQL fan", base, avatarUrl = "https://i.pravatar.cc/150?u=alice"),
            User(2, "bob", "Bob", null, base.plusDays(1), avatarUrl = "https://i.pravatar.cc/150?u=bob"),
            User(3, "charlie", "Charlie", "lurker", base.plusDays(2), avatarUrl = "https://i.pravatar.cc/150?u=charlie"),
        ).forEach {
            users[it.id] = it
    ...
    fun findAllById(ids: Collection<Long>): List<User> = ids.mapNotNull { users[it] }

    fun findByUsername(username: String): User? = byUsername[username]?.let { users[it] }
